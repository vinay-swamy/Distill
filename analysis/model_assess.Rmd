...
library(tensorflow)
library(PRROC)
### add so make it easier to save hm's###
library(ggplotify)
library(glue)
```
...
load('clean_data/allX_2020_06_24_VS.Rdata')
fig_outdir <- 'figures/vs_rerun_v1/'
allX$primatedl  <- as.numeric(allX$primatedl)
allX[is.na(allX)] <- -1

load('clean_data/assess_2020_06_24_VS.Rdata')
assess_set$primatedl  <- as.numeric(assess_set$primatedl)
assess_set[is.na(assess_set)] <- -1
#gdata::keep(allX, assess_set, VPaC_12mtry_v11, fig_outdir,j,sure = T) # VS: to reset 
```
...
dat <- assess_set %>% filter(DataSet == 'SuperGrimm') %>% 
  select(one_of(numeric_predictors, 'Distill','primatedl')) %>%  
  mutate_at(vars(one_of(numeric_predictors, 'Distill','primatedl')), lst(as.numeric)) %>% 
  select(heatmap_predictors)
#rem <- nearZeroVar(dat)
#dat <- dat[,-rem]
allpred_hm <- superheat(cor(dat),
          pretty.order.rows = T, 
          pretty.order.cols = T, 
          left.label = 'variable', 
          force.left.label=T, 
          ...
          left.label.size = 0.2) %>% .[['plot']] %>% as.ggplot
allpred_hm
ggsave(glue('{fig_outdir}all_predictors_cor.png'),
       plot = allpred_hm, height = 10, width = 10)
```
...
# remove near zero var predictors with caret's nearZeroVar
dat1 <- assess_set %>% filter(DataSet == 'SuperGrimm') %>% 
  select(one_of(numeric_predictors, 'Distill','primatedl')) %>%  
  mutate_at(vars(one_of(numeric_predictors, 'Distill','primatedl')), lst(as.numeric)) %>% 
  select(heatmap_predictors) 
colnames(dat1) <- gsub('_rankscore|_score|_converted','',colnames(dat1))
#rem <- nearZeroVar(dat)
#dat <- dat[,-rem]
#dat1[is.na(dat1)] <- 0 # vs: added this so it can run
pop_af_hm <- superheat(cor(dat1),
...
ggsave(glue('{fig_outdir}pop_af_cor_hm.png'),
       plot = pop_af_hm, height = 10, width = 10)
...
gini_plot <- VPaC_12mtry_v11$importance %>% data.frame() %>% rownames_to_column('Predictor') %>% 
         mutate(Predictor = gsub('_rankscore|_score|_converted','',Predictor)) %>% 
         arrange(MeanDecreaseGini) %>% 
         tail(20) %>% 
         mutate(Predictor = factor(Predictor, levels=unique(Predictor))) %>% 
         ...
         theme(text=element_text(family = 'mono'))
gini_plot
ggsave(filename =  glue('{fig_outdir}mean_decrease_gini_top20.svg'),
       gini_plot,
       height=3,
       width=4)
```
...
```{r, fig.height=1.5, fig.width=3}
mda_plot <- VPaC_12mtry_v11$importance %>% data.frame() %>% rownames_to_column('Predictor') %>% 
  mutate(Predictor = gsub('_rankscore|_score|_converted','',Predictor)) %>% 
  ...
  theme(text=element_text(family = 'mono'))
mda_plot
ggsave(filename = glue('{fig_outdir}mean_decrease_accuracy_top20.svg'),
       mda_plot,
       height=3,
       width=4)
...
model_vec <- c('DeepRNN',
               'VPaC', 'Distill', 'xgbTree','revel','cadd_phred', 'primatedl', 'fitcons', 'metalr_rankscore', 'fathmm_converted_rankscore', 'm_cap_rankscore')
allX_predictions <- allX[,c('DataSet', model_vec)]
allX_missing <-allX_predictions %>%  filter(DeepRNN <=0 | VPaC <=0 |xgbTree <=0)
...
for (i in model_vec){
  ...
  for (j in unique(assess_set %>% filter(DataSet != 'Extra NotPathogenic') %>%
    ...
    # datasets <- c("SuperGrimm", "Test Set", #"DDL NISC RD Cohort",
    # "Homsy",  "Samocha", "Wellderly", "Colombia Exomes", "UK10K Withheld" )
    # for(j in datasets){
    #print(j)
    # Samocha has way more path than not path, so use flipped pr_maker
    # predictor = i
    #data=  assess_set %>% filter(DataSet == j)
    if (j=='Samocha'){
...
pr_table <- pr_data %>% mutate(Model = factor(Model, levels = model_vec)) %>% 
  ...
  spread(Model, AUC) #%>% 
write_tsv(pr_table,  glue('{fig_outdir}pr_table.tsv') )
...
roc_table <- roc_data %>% mutate(Model = factor(Model, levels = model_vec)) %>% 
  ...
  mutate(AUC=round(AUC,4)) %>% 
  spread(Model, AUC) #%>% 
write_tsv(roc_table, glue('{fig_outdir}roc_table.tsv'))  

pr_table %>% DT::datatable()

roc_table %>% DT::datatable()
```
## Dot plots of AUC
```{r, fig.height=6}
...
ggsave(filename = glue('{fig_outdir}homsy_samocha_roc.svg'), cowplot::plot_grid(homs, sam, ncol = 1), width = 6, height = 6)
...
ggsave(filename = glue('{fig_outdir}pr_assess_set.svg'), cowplot::plot_grid(test_set, uk10k, colombia, ddl, superG, ncol=2), width = 12, height = 8)
...
Gpr_table <- Gpr_data %>% mutate(Model = factor(Model, levels = model_vec)) %>% 
  ...
  mutate(AUC=round(AUC,4)) %>% 
  spread(Model, AUC) 
write_tsv(Gpr_table, glue('{fig_outdir}gpr_table.tsv'))
Groc_table <- Groc_data %>% mutate(Model = factor(Model, levels = model_vec)) %>% 
  ...
  mutate(AUC=round(AUC,4)) %>% 
  spread(Model, AUC) 
write_tsv(Groc_table, glue('{fig_outdir}groc_table.tsv'))
Gpr_table%>% 
  DT::datatable()
...
groc_data_plot <- Groc_data %>%
  ...
  theme(text=element_text(family = 'mono'))
ggsave(glue('{fig_outdir}groc_data_plot.png'), groc_data_plot, width = 5, height = 4)
```
...
optimal_scoring_plot <- optimal_scoring %>% 
  mutate(Model = gsub('_rankscore|_score|_converted','', Model)) %>% 
...
ggsave(filename = glue('{fig_outdir}wellderly_performance.svg'), optimal_scoring_plot, height=1,width=3.5,
       )

```


# OGVFB Exomes - this is broken, and not sure what can fix it 
```{r}

ogvfb$ogvfb_ML_set$VPaC <-predict(VPaC_12mtry_v11, ogvfb$ogvfb_ML_set %>% select(any_of(c(nn_predictors, numeric_predictors)) ), type='prob')[,1]
