...
library(stringr)
library(data.table)
grimm_csvs <- list.files('data/raw_prep/', pattern = 'tool_scores.csv', full.names = T, recursive = T)
...
sum(is.na(grimm_data$CHR))
grimm_data_collapse <- grimm_data %>% 
  group_by(CHR, `Nuc-Pos`, `REF-Nuc`, `ALT-Nuc`) %>% 
  summarise(Status = paste0('Status=',paste(unique(Status), collapse='__')),
